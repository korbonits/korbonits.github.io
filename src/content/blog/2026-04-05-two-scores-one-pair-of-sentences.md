...
The answer goes back to the **distributional hypothesis**, articulated by the linguist J.R. Firth in 1957: *"You shall know a word by the company it keeps."* <sup>[Firth, J.R. (1957). "A synopsis of linguistic theory 1930-55." *Studies in Linguistic Analysis*.]</sup> Words that appear in similar contexts — surrounded by similar other words — tend to have similar meanings. Modern embedding models generalize this from words to full sentences and documents, trained on hundreds of millions of (text, similar-text) pairs using contrastive objectives: pull semantically similar pairs together, push dissimilar pairs apart.
...
Three patterns that consistently produce large gaps. Real scores from the live tool:
...
Claude produces outputs that are better calibrated to human semantic judgment on those cases. Whether that's because it has "world knowledge" or "understands truth conditions" is genuinely unclear — it's also a statistical model trained on text. What's clear is that it behaves differently, and the architectural reasons for that are tractable: joint attention over both texts, training on explicit reasoning tasks, autoregressive generation that allows deliberate comparison. It also can't run in a browser at query time, doesn't scale to retrieval over a large corpus without an inference call per comparison, and at nonzero temperature will give you a different answer if you ask the same question twice.

They're tools for different jobs. The interesting cases are the ones where the jobs overlap — and where the tools disagree.
...
The divergence between cosine similarity and semantic judgment is overdetermined — several independent factors contribute.

**Anisotropy.** [Ethayarajh (2019)](https://arxiv.org/abs/1909.00512) showed that BERT-family embeddings are anisotropic: vectors cluster in a narrow cone of the high-dimensional space rather than spreading uniformly. This artificially inflates cosine similarities across the board — unrelated texts score higher than you'd expect. SimCSE ([Gao et al., 2021](https://arxiv.org/abs/2104.08821)) diagnosed this as representational collapse and proposed contrastive fine-tuning as a fix. Modern models like nomic-embed address it, but it's never fully eliminated. Notably, anisotropy is partly a *consequence* of the training objective — the next two factors are related rather than independent.

**Training objectives don't encode contradiction.** Most embedding models are trained on positive pairs: paraphrases, adjacent sentences, query-document pairs. The contrastive signal pulls similar things together but never explicitly teaches "these two sentences assert opposite things." Hard negative mining — including syntactically similar but semantically opposite pairs — helps at the margins but is rarely comprehensive enough to cover negation and antonymy reliably.

**The encoder compression bottleneck.** Encoders map a full text into a single fixed-length vector, discarding relational structure in the process. A decoder attending over full context can track "this sentence says X, that one says not-X." A fixed embedding vector has no slot for that kind of relational information. This is also why the LLM-as-embedder direction is promising — decoder-derived representations carry more of that structure into the vector space.

**The LLM-as-embedder direction.** Recent work — [LLM2Vec](https://arxiv.org/abs/2404.05961) (BehnamGhader et al., 2024), E5-mistral, NV-Embed — converts decoder LLMs into embedding models and closes part of the gap, particularly on tasks requiring semantic reasoning. The hypothesis is that autoregressive training forces models to build something closer to a world model, not just distributional statistics. The gap narrows but doesn't disappear: cosine similarity over any fixed vector is still a lossy approximation of judgment.

The practical implication: if your application cares about *what texts mean* rather than *what topics they cover* — contradiction detection, claim verification, legal semantics — cosine similarity alone is the wrong tool. It's a fast, scalable proxy for topical proximity, and a poor proxy for semantic agreement.
